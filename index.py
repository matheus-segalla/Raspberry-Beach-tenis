...
import time
import json
import requests  # Trocamos a biblioteca do Supabase pelo requests nativo
...
# Configurações da API do Supabase vindas do seu .env
...
print("=== SISTEMA DE REPLAY VIA HTTP (COMPATÍVEL PYTHON 3.14) ===")
print("Pressione [ESPAÇO] para salvar uma jogada. Pressione [ESC] para sair.\n")

def iniciar_buffer_circular():
    """Inicia o FFmpeg em segundo plano para gravar a webcam em pedaços de 5s."""
    if os.name == 'nt':  # Windows
        ...
        cmd_input = ['-f', 'avfoundation', '-i', '0']

    comando = [
        'ffmpeg.exe', *cmd_input,
...
def processar_fila_de_replays():
    ...
    try:
        ...
        if len(arquivos) >= 3:
            ...
            comando_concat = ['ffmpeg.exe', '-f', 'concat', '-safe', '0', '-i', lista_txt_path, '-c', 'copy', '-y', arquivo_final]
...
def executar_upload_da_fila():
    ...
    try:
        ...
        if resposta_storage.status_code == 200:
            print("💾 [STORAGE] Arquivo salvo no bucket com sucesso!")
            
            # Monta a URL pública definitiva do arquivo de vídeo mp4
            url_publica_video = f"{SUPABASE_URL}/storage/v1/object/public/replays/{nome_arquivo_nuvem}"
            
            # 2. INSERE O REGISTRO NA TABELA 'REPLAYS' PARA GERAR O LINK DO JOGADOR
            ...
            headers_banco = {
                ...
                "Prefer": "return=representation"  # Força o banco a devolver os dados inseridos (incluindo o UUID do replay)
            }
            
            payload_banco = {
                "quadra_id": os.getenv("QUADRA_ID"),
            ...
            if resposta_banco.status_code == 201:
                ...
                id_do_replay = dados_replay_criado[0]['id'] # Pega o ID único gerado pelo banco de dados
                
                # Monta o link apontando para o seu Front-end local (Dashboard/Tela do Jogador)
                link_front_jogador = f"http://localhost:3000/jogada?id={id_do_replay}"
                
                print(f"🚀 [SUCESSO TOTAL] Replay disponível na nuvem!")
                print(f"👉 Link do Jogador: {link_front_jogador}\n")
                
                # --- GERAÇÃO DO QR CODE APONTANDO PARA A PÁGINA DO JOGADOR ---
                print("🖼️  [QR CODE] Gerando imagem direcionada ao app...")
                
                qr = qrcode.QRCode(version=1, box_size=10, border=5)
                qr.add_data(link_front_jogador) # Link agora vai para a página do Next.js
                qr.make(fit=True)
    ...
    with open(ARQUIVO_FILA, "w") as f: json.dump(fila, f, indent=4)
# 1. Inicia a gravação contínua da câmera
